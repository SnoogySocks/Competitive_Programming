...
void solve () {
    ...
    // Is it more expensive to make 1 library and connect
    // them all, or make a library for everyone?
    for (int i = 1; i<=c; ++i) {
        if (vis[i]) continue;
        q.push(i);
        vis[i] = true;
        ...
        // Compare the used_roads cost with the used_lib cost
        cost += min(c_lib+c_road*used_roads, c_lib*(used_roads+1));
    }
    print(cost);
}
