...
void solve() {
    ...
    int R, C; scan(R, C);
    vec<vec<int>> laboratory(R+1, vec<int>(C+1));
    int k; scan(k);
    const int hasCat = -1;
    while (k--) {
        int r, c; scan(r, c);
        laboratory[r][c] = hasCat;
    }
    ...
    for (int r = 1; r<=R; ++r) {
        for (int c = 1; c<=C; ++c) {
            if (laboratory[r][c]!=hasCat&&(r!=1||c!=1)) 
                laboratory[r][c] = max(laboratory[r][c-1], 0)+max(laboratory[r-1][c], 0);
        }
    }

    print(laboratory[R][C]);
