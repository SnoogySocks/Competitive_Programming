...
/*
Number of ways to count numPeople sorted numbers that sum to numPi
*/

void solve() {

    int numPi, numPeople; scan(numPi, numPeople);
    numPi -= numPeople;
    // dp[numPi][numPeople] = num ways to sum to numPi with numPeople
    vec<vec<int>> dp(numPi+1, vec<int>(numPeople+1));
    for (int i = 1; i<=numPeople; ++i) {
        dp[0][i] = 1;
    }

    for (int people = 1; people<=numPeople; ++people) {
        for (int pi = 1; pi<=numPi; ++pi) {
            dp[pi][people] = dp[pi][people-1];
            if (pi-people>=0) dp[pi][people] += dp[pi-people][people];
        }
    }
    
    print(dp[numPi][numPeople]);
