...
/*
square size   max people
1             2
2             5
3             8
4             13
5             18
start at 1, add i on oddsquare sizes
if it is even, increment i by 2 then add i
...
void solve() {
    ...
    int n; scan(n);
    int max, tracker, size;
    max = 1;
    tracker = 1;
    for (size = 1; max<n; ++size) {
        if (size%2==0) tracker+=2;
        max += tracker;
    }
    print(size-1);
