...
void solve () {
    ...
    // Find the left most and rightmost indeces of each station
    for (int i = 0; i<n; ++i) {
        if (!routes.count(stations[i])) {
            routes[stations[i]] = make_pair(i, i);
        } else {
            pii& route = routes[stations[i]];
            route = make_pair(
                    min(route.first, i),
                    max(route.second, i)
                );
    ...
    while (k--) {
        int a, b; scan(a, b);
        puts(routes.count(a) && routes.count(b) && routes[a].first<routes[b].second ? "YES" : "NO");
    }
